...
impl MapBuilder {
    ...
    fn fill_edges(&mut self) {
        for y in 1 .. SCREEN_HEIGHT - 1 {
            let idx1 = map_idx(SCREEN_WIDTH - 1, y);
            let idx2 = map_idx(1, y);
            self.map.tiles[idx1] = TileType::Wall;
            self.map.tiles[idx2] = TileType::Wall;
        }

        for x in 1 .. SCREEN_WIDTH - 1 {
            let idx1 = map_idx(x, 1);
            let idx2 = map_idx(x, SCREEN_HEIGHT - 1);
            self.map.tiles[idx1] = TileType::Wall;
            self.map.tiles[idx2] = TileType::Wall;
        }
    }
